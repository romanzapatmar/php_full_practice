...
$con = mysqli_connect('localhost','root','','php') or die('Unable To connect');
if(count($_POST)>0) {
$result = mysqli_query($con,"SELECT *from mytable WHERE username='" . $id . "'");
$row=mysqli_fetch_array($result);
if($_POST["currentPassword"] == $row["password"] && $_POST["newPassword"] == $row["confirmPassword"] ) {
mysqli_query($con,"UPDATE mytable set password='" . $_POST["newPassword"] . "' WHERE username='" . $id . "'");
